...
if($value == 'active'){
    $user_id = $this->db->get_where('listing', array('id' => $listing_id))->row('user_id');
$email = $this->db->get_where('user', array('id' => $user_id))->row('email');
$name = $this->db->get_where('user', array('id' => $user_id))->row('name');

    $this->email_model->sent_smtp_mail_with_php_mailer_library( "Dear " .$name ."\n
thank you for listing your website.
Your Listing is Approved by Connecting Hindu Globally.
you can go to check your listing on our website. https://chg.vasero.in/aaa/home/listings", "Listing Conformation", $email, "test@gmail.com");
}
